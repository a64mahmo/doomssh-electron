...
- **Example resumes:** The Resumes dashboard always renders the same layout: heading, grid (with the New Resume card), then `ExampleResumes` (three examples and tips) until the user hides it (`localStorage` key `doomssh:examples-hidden`; "Browse examples" restores it).
