...
- **Example resumes:** The Resumes dashboard always renders the same layout: heading, grid (with the New Resume card), then `ExampleResumes` (three examples and tips) until the user hides it (`localStorage` key `doomssh:examples-hidden`; "Browse examples" restores it). Hide/show animate with `framer-motion` (skipped under `prefers-reduced-motion`), hiding raises an Undo toast, and focus moves to "Browse examples" or the section heading.
