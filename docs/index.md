...
-   **21 Templates:** Setting presets from compact one-pagers to photo, header-band and creative layouts, all fully customizable.
-   **Cross-Platform:** Available as a web application (Cloudflare) and a desktop application for macOS, Windows, and Linux.
-   **Local-First Storage:** JSON files in a vault on desktop; IndexedDB (Dexie.js) in the browser.
-   **PDF Export:** The desktop app prints the live HTML preview with Chromium, so the PDF matches what you see. The web build prints the same page through the browser's print dialog (Save as PDF).
...
-   **Local Database:** Vault JSON files via Electron IPC (desktop); Dexie.js / IndexedDB (web)
-   **PDF:** The HTML template printed — Chromium `printToPDF` (desktop); browser print dialog (web)
-   **AI Integration:** Anthropic SDK via Electron IPC (Claude Opus 4.6), desktop only
...
-   [Electron Integration](./electron.md) - Packaging, PDF export and desktop-specific features.
-   [Template Customization](./template-customization.md) - Settings reference, built-in presets and adding templates.
-   [Deployment](./deployment.md) - Deploying the web build to Cloudflare and the platform build variable.
...
-   **HTML → PDF export (desktop):** The desktop app now renders the resume on the `/print` page in a hidden window and saves it with Chromium's `printToPDF`, so exported PDFs match the live preview exactly. See [PDF Export](./architecture.md#pdf-export-html--pdf).
-   **Live HTML preview:** The preview renders the HTML template directly instead of regenerating a PDF in an iframe — edits appear instantly, with no flicker or scroll reset, plus approximate page guides.
-   **Web build:** Resumes, cover letters and jobs are stored in IndexedDB when running in a browser. Deployed to Cloudflare as static assets ([Deployment](./deployment.md)).
-   **Platform flag:** `NEXT_PUBLIC_APP_PLATFORM` (`web` / `electron`) and `frontend/lib/platform.ts` separate browser and desktop behaviour. The browser Settings dialog hides AI, Bug Mode and update controls.
-   **Six new templates:** Aspen (photo sidebar), Vega (compact), Lumen (spacious), Atlas (header band), Sierra (creative) and Nova (executive), with thumbnails that reflect each preset.
-   **PDF layout fixes:** No more blank trailing pages, stranded headings, contact lines starting with a separator, overflowing names or skill pills, or large gaps from entries that refused to split.
-   **Fixed the job dialog in Safari:** The New Application form no longer collapses behind its footer.
-   **Fixed lost saves:** Resume and job edits made in quick succession could be dropped. Saving now watches the document itself and re-saves changes made during a save.
-   **Fixed missing section content:** Award titles and dates, volunteering roles, reference names and certification/publication dates now render.
-   **New logo:** A folded-corner D mark for the favicon, app icons and in-app branding.
