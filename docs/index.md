...
-   **PDF Export:** The desktop app prints the live HTML preview with Chromium, so the PDF matches what you see. The web build downloads a PDF generated by `@react-pdf/renderer`.
...
-   **PDF:** Chromium `printToPDF` (desktop); `@react-pdf/renderer` (web download)
