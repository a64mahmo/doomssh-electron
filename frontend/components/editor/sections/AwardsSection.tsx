...
import { Plus, Trash2, Copy, Award, Building2, Calendar } from 'lucide-react'
...
export function AwardsSection({ sectionId }: Props) {
  const { section, updateItems } = useSection(sectionId)
  const items = (section?.items as AwardItem[]) || []
  const { improveText } = useAI()

  function add() {
    updateItems([{ id: generateId(), title: '', issuer: '', date: '', description: '' }, ...items])
  }

  function update(id: string, changes: Partial<AwardItem>) {
  ...
  async function handleAIImprove(id: string, text: string, title: string) {
    ...
    try {
      const improved = await improveText(text, `Award: ${title}`)
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <span className="font-bold uppercase tracking-wider text-muted-foreground/80">Add Award</span>
      </Button>
    </div>
  )
}
