...
export function ProjectsSection({ sectionId }: Props) {
  ...
  function add() {
    updateItems([...items, { id: generateId(), name: '', url: '', date: '', description: '' }])
  }
    ...
    <div className="space-y-3">
      {items.map((item) => (
        <div key={item.id} className="rounded-md border border-border p-3 space-y-2.5 relative">
          ...
          <Field label="URL">
            <Input className="h-7 text-xs" placeholder="https://github.com/..." value={item.url} onChange={(e) => update(item.id, { url: e.target.value })} />
          </Field>
