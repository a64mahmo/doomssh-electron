...
export function ProjectsSection({ sectionId }: Props) {
  const { section, updateItems } = useSection(sectionId)
  const items = (section?.items as ProjectItem[]) || []
  const { improveText } = useAI()

  function add() {
    updateItems([{ id: generateId(), name: '', url: '', date: '', description: '' }, ...items])
  }

  function update(id: string, changes: Partial<ProjectItem>) {
  ...
  async function handleAIImprove(id: string, text: string, projectName: string) {
    ...
    try {
      const improved = await improveText(text, `Project: ${projectName}`)
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-4">
        {items.map((item, index) => (
          ...
          >
            ...
            <div className="pt-2">
              <Field label="Description & Key Contributions">
                <RichTextArea
                  placeholder="• Developed using React and Next.js&#10;• Implemented real-time updates using WebSockets..."
                  value={item.description}
                  onChange={(v) => update(item.id, { description: v })}
                  onAIImprove={() => handleAIImprove(item.id, item.description, item.name)}
                />
              </Field>
            </div>
          </div>
      ...
      >
        ...
        <span className="font-bold uppercase tracking-wider text-muted-foreground/80">Add Project</span>
      </Button>
    </div>
  )
}
