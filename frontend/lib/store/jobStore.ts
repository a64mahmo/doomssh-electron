...
import { JOB_STATUS_LABELS } from "@/lib/store/jobTypes";
import {
  loadAllJobs as dbLoadAllJobs,
  saveAllJobs as dbSaveAllJobs,
  ...
  subscribeWithSelector(
    ...
    loadJobs: async () => {
      ...
      jobs.forEach((job) => {
        if (job.deadlineDate && job.deadlineDate < today) {
          const alreadyAdded = job.events.some(
            (e) => e.type === "deadline-passed",
          );
          if (!alreadyAdded) {
            job.events.push({
              id: generateId(),
    ...
    updateJob: (jobId, updates) =>
      set((state) => {
        ...
        if (
          "deadlineDate" in updates &&
          updates.deadlineDate !== job.deadlineDate
        ) {
    ...
    moveJob: (jobId, newStatus: JobStatus) =>
      set((state) => {
        ...
        if (
          ["phone-screen", "technical", "onsite"].includes(newStatus) &&
          !job.responseDate
        ) {
    ...
    updateInterviewPrep: (jobId, prep) =>
      set((state) => {
        const job = state.jobs.find((j) => j.id === jobId);
        if (!job) return;
        const existing: InterviewPrep = job.interviewPrep ?? {
          questions: [],
          companyNotes: "",
          cheatSheet: [],
          reflections: [],
